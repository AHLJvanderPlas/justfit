...
// ─── TRAINER INVITE SCREEN ────────────────────────────────────────────────────
// Full-screen overlay shown when user navigates to /trainer-invite?t=<token>
function TrainerInviteScreen({ token, inviteToken }) {
...
// ─── CONNECT SCREEN ───────────────────────────────────────────────────────────
// Full-screen overlay shown when user navigates to /connect?t=<trainer_token>
function ConnectScreen({ token, connectToken }) {
  const [info, setInfo] = useState(null);
  const [loading, setLoading] = useState(true);
  const [acting, setActing] = useState(false);
...
// ─── PENDING INVITE MODAL (post-onboarding) ───────────────────────────────────
// Shown after new-user onboarding completes when signup URL had ?invite=<token>
function PendingInviteModal({ token, inviteToken, onDone }) {
  const [info, setInfo] = useState(null);
  const [acting, setActing] = useState(false);
...
export default function App() {
  ...
  if (_appPath === '/trainer-invite' && _appParams.get('t')) {
    return <TrainerInviteScreen token={token} inviteToken={_appParams.get('t')} />;
  }
  if (_appPath === '/connect' && _appParams.get('t')) {
    return <ConnectScreen token={token} connectToken={_appParams.get('t')} />;
  }
