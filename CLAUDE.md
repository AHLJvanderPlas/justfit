...
| Login page as marketing — Overhaul 20 (May 2026) | ✅ Live — `public/login.html` restructured into two zones: Zone 1 (auth card, 100dvh, unchanged behaviour) + Zone 2 (product explainer below fold); Google Fonts (Barlow Condensed / Inter Tight / JetBrains Mono) added; animated scroll cue chevron between zones; three pillars (Bolt/Compass/Shield SVG icons, accent colour); static Today card mockup (Cooper Test session); "What we don't do" em-dash mono list; single pricing sentence; "Back to sign in →" smooth-scroll link; all auth JS untouched; scroll cue respects prefers-reduced-motion |
