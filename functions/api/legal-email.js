...
function getPrivacyEmail() {
  const body = [
    ...
    section('4. Data Retention', 'Data is retained while your account is active. Account deletion removes all data within 24 hours; backups purged within 30 days.\n\nInactive accounts are currently retained until user-requested deletion. If automated inactivity deletion is introduced in the future, users will be notified in advance and this policy will be updated before it takes effect.'),
    ...
    section('10. Policy Changes', 'If material changes are made, version/effective date will be updated and notice provided in-app and/or by email where appropriate. For major legal updates, users may be asked to re-accept updated terms/privacy in-app before continuing. Previous versions are available on request.'),
...
function getTermsEmail() {
  const body = [
    ...
    section('11. Changes to Terms', 'Material changes are notified in-app and/or by email before or when they take effect, depending on the change. Continued use after the effective date = acceptance where permitted by law. For major legal updates, explicit in-app re-acceptance may be required before continued use.'),
    ...
    section('13. Contact', 'support@justfit.cc\n\nFor legal notices or requests for formal legal entity and registered-office details, contact support@justfit.cc.'),
  ].join('');
...
function getMissionEmail() {
  const body = [
    section('Version 1.1 — April 2026', 'What JustFit stands for and the principles that guide every product decision.'),
    section('Our Vision', 'Make adaptive, trustworthy personal coaching available every day — without requiring perfect motivation, perfect health, or perfect circumstances.'),
    section('Our Mission', 'Help people stay consistently active by turning real-life constraints into safe, clear, daily training decisions.\n\nSleep bad? Stressed? Travelling? Injured? JustFit adjusts so you still move — safely, appropriately, and without skipping days.'),
    section('Product Principles', '1. Consistency beats intensity — A completed session is better than a perfect session that gets skipped.\n2. Real life outranks plan purity — Sleep, stress, pain, travel, schedule, and recovery signals may immediately change the session.\n3. Safety beats ambition — The system must prefer safe downgrades over risky progression.\n4. The planner must be explainable — Meaningful plan changes are traceable to understandable rules.\n5. One active training intent at a time — The engine always knows the single primary thing it is optimising for today.\n6. Privacy is part of the product — Data minimisation, explicit consent, and user control are core product behaviour.\n7. During training, speed wins — Workout interactions are one-thumb, glanceable, and usable in about 2 seconds.'),
    section('What JustFit Is — and Is Not', 'JustFit is: a privacy-first adaptive daily training coach, a deterministic rule-based planner, and a consistency tool.\n\nJustFit is not: a social fitness network, a medical device or advisor, a black-box AI coach, or a max-performance-at-any-cost app.'),
    ...
    section('Privacy First', 'Your fitness data is yours. Identity and health data are separated at the database level. We do not sell data and do not profile users for ads. A minimal set of processors is used to operate the service, documented in the Privacy Policy.'),
    section('Contact', 'Questions or feedback on our philosophy: support@justfit.cc'),
  ].join('');
  return emailShell('Mission, Vision &amp; Philosophy', 'Version 1.1 &mdash; April 2026', body);
}
